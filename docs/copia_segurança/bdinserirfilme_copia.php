...
$cartaz = $_POST["emCartaz"]; 
	...
	if (empty($nome) ||  empty($nomeOriginal) ||  empty($estreia) || empty($duracao) ||  empty($genero) ||  empty($paisOrigem) ||  empty($direcao) ||  empty($elenco) || empty($sinopse) ||  empty($imgPoster) ||  empty($cartaz)){
